...
$ALGORITHMFILE = "$PSScriptRoot\Get-WordOrder_ComboScoringOptimized.ps1"
...
#---------------------Run different algorithm if possible guess list is too big--------------------------------

if($PossibleWords.count -gt 20){
    ...
    & $AlgorithmFile -PossibleWords $possiblewords -WordsArray $resultsArray
}
else{
    Get-WordOrder_Lookahead -PossibleWords $possiblewords -WordsArray $resultsArray -OptimizeFor "mean"
}